//! HTML5 named character references recognized by CommonMark.
//!
//! Generated data --- the WHATWG/HTML5 named-entity set filtered to the
//! semicolon-terminated names (CommonMark requires the trailing `;`), sorted
//! by name for binary search. Source: Python 3 `html.entities.html5` (the
//! canonical WHATWG list cmark/roxygen2 resolve against). Values escape every
//! non-ASCII code point (`\u{...}`) so the table carries no invisible characters.
//! Do not hand-edit; regenerate if the reference list changes.

/// `(name, replacement)` for every semicolon-terminated HTML5 named character
/// reference, sorted by `name` (the text between `&` and `;`). Values may be
/// more than one code point (93 of them are).
