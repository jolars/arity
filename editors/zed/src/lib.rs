...
impl ArityExtension {
    fn language_server_binary(
        ...
        if let Some(path) = worktree.which("arity") {
            return Ok(ArityBinary {
                path,
                args: binary_args,
            });
        ...
        // in the repository (`arity-parser-v*`, `arity-formatter-v*`,
        // `arity-code-v*`, `arity-zed-v*`) must stay asset-free, or it would
        // shadow the CLI release here. See AGENTS.md "Releases".
        ...
        // The host returns the tag name verbatim (e.g. `v0.20.0`); strip the `v`
        // so the cached directory reads `arity-0.20.0`.
        let version = release
            .version
            .strip_prefix('v')
        ...
        let asset = release_details
            ...
            .ok_or_else(|| {
                format!(
                    "Arity release {} has no asset matching any of {:?}",
...
impl GithubReleaseDetails {
    fn new(platform: zed::Os, arch: zed::Architecture, version: String) -> Result<Self> {
        let arch = match arch {
            ...
            // Arity publishes no 32-bit assets, so failing here beats asking the
            // host to download an `x86-*` archive that does not exist.
            zed::Architecture::X86 => {
                return Err("Arity does not publish binaries for 32-bit x86".into())
            }
        };

        let asset_names = match platform {
            zed::Os::Mac => vec![format!("arity-{arch}-apple-darwin.tar.gz")],
            zed::Os::Linux => vec![
                format!("arity-{arch}-unknown-linux-gnu.tar.gz"),
                format!("arity-{arch}-unknown-linux-musl.tar.gz"),
            ],
            zed::Os::Windows => vec![format!("arity-{arch}-pc-windows-msvc.zip")],
        };
        ...
        let downloaded_directory = format!("arity-{version}");
        ...
        let downloaded_binary_path = match platform {
            zed::Os::Mac | zed::Os::Linux => format!("{downloaded_directory}/arity"),
            zed::Os::Windows => format!("{downloaded_directory}/arity.exe"),
        };
...
impl zed::Extension for ArityExtension {
    fn new() -> Self {
        Self {
    ...
    fn language_server_command(
        ...
        let arity_binary = self.language_server_binary(language_server_id, worktree)?;
        Ok(zed::Command {
            command: arity_binary.path,
            // `arity lsp` takes no arguments of its own: `Commands::Lsp` is a unit
            ...
            args: arity_binary.args.unwrap_or_else(|| vec!["lsp".into()]),
            env: vec![],
        })
    }
...
zed::register_extension!(ArityExtension);
...
mod test {
    ...
    fn resolves_macos_release() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec![String::from("arity-aarch64-apple-darwin.tar.gz")],
                downloaded_file_type: DownloadedFileType::GzipTar,
                downloaded_directory: String::from("arity-0.1.0"),
                downloaded_binary_path: String::from("arity-0.1.0/arity"),
            })
        );
    }
    ...
    fn resolves_linux_release_with_musl_fallback() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec![
                    String::from("arity-x86_64-unknown-linux-gnu.tar.gz"),
                    String::from("arity-x86_64-unknown-linux-musl.tar.gz"),
                ],
                downloaded_file_type: DownloadedFileType::GzipTar,
                downloaded_directory: String::from("arity-0.2.0"),
                downloaded_binary_path: String::from("arity-0.2.0/arity"),
            })
        );
    }
    ...
    fn resolves_windows_release() {
        assert_eq!(
            ...
            Ok(GithubReleaseDetails {
                asset_names: vec![String::from("arity-x86_64-pc-windows-msvc.zip")],
                downloaded_file_type: DownloadedFileType::Zip,
                downloaded_directory: String::from("arity-0.1.0"),
                downloaded_binary_path: String::from("arity-0.1.0/arity.exe"),
            })
        );
    }
