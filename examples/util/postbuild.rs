...
/// So are the stubs mdBook writes for `[output.html.redirect]`, which stand in
/// for a page that moved and whose content now lives at the target URL.
...
/// Whether `path` is one of mdBook's redirect stubs rather than a content page.
/// The stub's whole job is the `<meta http-equiv="refresh">` it carries, which
/// no rendered chapter has; an unreadable file is treated as content, matching
/// the rest of this module's best-effort stance.
fn is_redirect(path: &Path) -> bool {
    std::fs::read_to_string(path).is_ok_and(|html| html.contains(r#"http-equiv="refresh""#))
}
