...
/// backslash-escaped `\[` or `\]` is content (mirrors `is_shortcut_content` in
/// the lexer — `double_escape_md`'s bracket-escape reverts keep both live
/// through cmark, so `[Foo*bar\]]` is a valid label, cm-196).
