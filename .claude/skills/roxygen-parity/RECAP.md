...
- **A `\` carve is parity-gated: parse_Rd pairs backslashes left-to-right.** A `\` preceded by
  ...
  the same k either way. `build_rd_content` (in-arg sub-parse) is NOT gated — in-arg parity +
  in-arg text-escape resolution are backlog.
...
- **md `\{`/`\}` render bare in TEXT, drop decision untouched (LANDED 2026-07-07b).** Under `@md` a lone
  ...
  strings and applies `unescape_lone_rd_brace` (drop a lone `\` before `{`/`}`) to **`TEXT` leaves only**.
  **VERB keeps `\{`** (verbatim `\verb`/`\url`/fenced code — engine-probed), and the resolution is
  ...
  `md_brace_escape`. **Backlog:** `\code`/RCODE brace resolution (parse_Rd resolves `\{`→`{` in an RCODE
  arg too — probed); **bare** `{ }` → Rd `(LIST …)` group (arity keeps flat TEXT); multi-backslash before
  a brace (`\\{`/`\\\{` — `unescape_lone_rd_brace` only consumes a lone `\`, mirroring
  `unescape_md_brackets`; note `collapse_md_backslash_runs` still mis-collapses `\\\{`→`\\{`, a latent
  wrong-drop).
...
corpus (132 `cm-NNN` cases). **Current: 389 matching (all allowlisted), 18 divergent** of 407 pinned.
...
1. **Projector parity** (`tests/roxygen_projector.rs`, pure Rust) — the **primary parser-growth
   ...
   Curated + harvested + spec corpora (407 pinned). The 18 divergences are out-of-scope.
2. **Curated fixed-point** (`tests/roxygen_oracle.rs::roxygen_oracle_report`, needs R, `#[ignore]`d) —
   strict semantic preservation of the formatter; 124/124 preserving, 0 blocked. *Meaning, not layout.*
...
## Latest session (2026-07-07b) — md `\{`/`\}` render bare in TEXT (escape-cluster slice 4)

md escaped-brace rendering landed, **projector-only, drop decision UNCHANGED**. Under `@md` a lone prose
escape `\{`/`\}` renders a **bare** brace (`a \{ b \} c`→`a { b } c`, unbalanced `a \{ b`→`a { b` still
kept, genuine bare `a { b` still dropped). Key realization: the `double_escape_md`→cmark round trip is a
**net no-op on a backslash-brace run** (source ≈ rendered Rd for the chars `rd_complete` weighs), so
arity's `@md` drop decision was **already correct** on the `\{`-preserving atoms — only the rendered
TEXT was wrong. Fix: a `@md`-gated post-pass in `project_block` (`resolve_md_text_braces`) runs **after**
every section (and its `rdComplete` drop) is built and applies `unescape_lone_rd_brace` to **`TEXT`
leaves only** (VERB keeps `\{`; quote-state-aware so a literal `(TEXT "` inside a code span is data).
Doing it in `process_prose` would feed the bare brace to the drop scan and false-drop `a \{ b` — the
post-pass sidesteps it. Full classification in the "md `\{`/`\}` render bare" trap above.

**Result:** projector **388→389 matching** (all allowlisted, 0 regressions), 18 divergent (unchanged,
out-of-scope). `cargo test` green (590 lib + all integration), clippy + fmt clean; curated fixed-point
**124/124 preserving** (ran, 0 blocked); format baseline **+1 additive** (re-blessed, reviewed).
New: curated `md_brace_escape` (+pin +allowlist); units `unescape_lone_rd_brace_resolves…`,
`resolve_md_text_braces_only_touches_text_leaves`, `markdown_on_escaped_brace_renders_bare_but_does_not_drop`.

**Ranked next target:** the escape-cluster remainder — **`\code`/RCODE brace resolution** (parse_Rd
resolves `\{`→`{` in an RCODE arg too, probed; TEXT-only post-pass skips it); **bare** `{ }`→Rd `(LIST …)`
group (arity keeps flat TEXT — the bare-brace-group model, also covers md bare `{y}`); multi-backslash
before a brace (`\\{`/`\\\{` — `collapse_md_backslash_runs` mis-collapses `\\\{`→`\\{`, a latent
wrong-drop); sticky brace-less mode-flips; in-arg `\`-parity + text-escape resolution in
`build_rd_content`; md `\{`/`\}` inside a *fragile* arg. Smaller list follow-ups (no-blank marker-type
split, blank + content-column prose folding (e4), block macro folding into an item, quote-lazy `===`).
