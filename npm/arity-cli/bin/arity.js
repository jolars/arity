...
const PACKAGES = {
  "linux-x64-gnu": "@arity-cli/linux-x64-gnu",
  "linux-arm64-gnu": "@arity-cli/linux-arm64-gnu",
  "linux-x64-musl": "@arity-cli/linux-x64-musl",
  "linux-arm64-musl": "@arity-cli/linux-arm64-musl",
  "darwin-x64": "@arity-cli/darwin-x64",
  "darwin-arm64": "@arity-cli/darwin-arm64",
  "win32-x64": "@arity-cli/win32-x64",
  "win32-arm64": "@arity-cli/win32-arm64",
};
...
function resolveCandidates() {
  ...
  if (!PACKAGES[keys[0]]) {
    throw new Error(
      `arity-cli does not ship a prebuilt binary for ${keys[0]}.\n` +
        `Supported platforms: linux (x64/arm64, gnu+musl), darwin (x64/arm64), win32 (x64/arm64).\n` +
        `See https://arity.cc for alternative install methods.`,
    );
  }
  ...
  if (candidates.length === 0) {
    throw new Error(
      `arity-cli expected the optional dependency ${PACKAGES[keys[0]]} to be installed, ` +
...
function loaderHint() {
  ...
  process.stderr.write(
    `The glibc build of arity cannot run on this system (no standard glibc loader; ` +
      ...
      `Or install arity via other means; ` +
      `see https://arity.cc.\n`,
  );
}
...
function main() {
  ...
  try {
    ...
    if (typeof err.status === "number") {
      ...
      // glibc build; probe to distinguish that from arity itself
    ...
    process.stderr.write(`Failed to execute arity: ${err.message}\n`);
    process.exit(1);
  }
}
