...
waits its turn (never cross-canceled, so a multi-URI relint still publishes
...
- `parsed_document` (`src/incremental.rs:80`)—`no_eq,   unsafe(non_update_types)`;
  caches `rowan::GreenNode` (`Send + Sync`,
...
- **Firewall queries** `file_exports` (`:130`), `file_free_reads` (`:141`),
  `source_edges` (`:156`)—return `Eq` values (`BTreeSet<String>` or
